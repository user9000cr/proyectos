...
classdef softmax < handle
  ...
  methods
    ...
    function g=backward(self,dJds)
      ...
      hadamard = self.outputs.*dJds;
      self.gradient = hadamard - ([(hadamard)*ones(columns(self.outputs),1)]*ones(1,columns(self.outputs))).*self.outputs;
      g=self.gradient;
    endfunction
  endmethods
endclassdef
