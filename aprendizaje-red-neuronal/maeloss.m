...
## "Capa" para calcular la pérdida con "MAE"
##
  ...
  methods
    ...
    function J=forward(self,Y,Ygt)
      ...
      elseif (isreal(Y) && ismatrix(Y) && (size(Y)==size(Ygt)))
        self.diff=Y-Ygt;
        self.outputs = (1/rows(Y))*sum(sum(abs(self.diff)));
        J=self.outputs;
        self.gradient = [];
      else
    ...
    function g=backward(self,dJds)
      ...
      #localGrad = (1/rows(self.diff)).*sign(self.diff);
      self.gradient = sign(self.diff)*dJds./(rows(self.diff));
